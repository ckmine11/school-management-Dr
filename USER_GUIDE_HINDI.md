...
### 3.12 School Settings — स्कूल की Settings

**Menu में जाएं → School Settings**

यहाँ से आप पूरे system का **branding** (दिखावट) बदल सकते हैं।

> ⭐ **जब भी नया client deploy करें, पहले यह page ज़रूर fill करें।**

---

#### School Branding (स्कूल की पहचान)

| Field | क्या भरें | कहाँ दिखता है |
|---|---|---|
| School Name | स्कूल का पूरा नाम | Login page, sidebar, PDF |
| Tagline | जैसे "Excellence in Education" | Login page के नीचे |
| Logo | स्कूल का logo (PNG/JPG) | Login page और sidebar में |
| Primary Color | Sidebar का color चुनें | Admin sidebar |
| Academic Year | जैसे 2024-25 | Student ID Card में |

#### Logo Upload करना

1. **Upload Logo** button दबाएं
2. अपने computer से logo file चुनें (PNG या JPG, max 2MB)
3. Logo upload होकर preview में दिखेगा
4. **Save Settings** दबाएं

> अगर logo हटाना हो तो **Remove** button दबाएं
...
| Field | क्या भरें |
|---|---|
| Address | स्कूल का पूरा पता |
| Phone | स्कूल का phone number |
| Email | स्कूल की email |
| Website | स्कूल की website (optional) |

यह जानकारी **Fee Receipt PDF** और **ID Card** में print होती है।
...
|---|---|---|
| Currency Code | INR | USD, GBP |
| Currency Symbol | ₹ | $, £ |

अगर स्कूल India में नहीं है तो यहाँ से currency बदल सकते हैं।

---

#### Fee Types (फीस के प्रकार)
...
**नई Fee Type जोड़ना:**
1. नीचे text box में type का नाम लिखें (जैसे "Hostel Fee")
2. **Add** button दबाएं या Enter दबाएं
3. List में जुड़ जाएगी

**Fee Type हटाना:**
- Type के सामने ❌ button दबाएं

> ये fee types **Fee Management → Add Fee** के dropdown में दिखती हैं

---

#### Settings Save करना

सब कुछ fill करने के बाद:
...
### बच्चे के Result देखना

**Menu → Child's Results**

- सभी परीक्षाओं के result एक साथ दिखेंगे
- हर exam का card: subject-wise marks, grade, percentage
- Overall Grade (A+, A, B+, B, C, D, F) और Pass/Fail status
- Class Rank (अगर admin ने enter किया है)
- Remarks (अगर teacher ने कुछ लिखा है)
...
- **Summary:** Total Fees / Paid / Due amount एक नज़र में
- हर fee record: Receipt No., Type, Amount, Due Date, Month, Status
- **Overdue fees** — Due Date निकल गई हो तो date लाल रंग में दिखेगी
- Filter: All / Unpaid / Paid / Partial
- View-only — payment Admin करता है, parent सिर्फ देख सकता है
...
## Client को Application देने से पहले — Checklist

यह checklist admin के लिए है जो application setup करके client को देता है।

### JWT_SECRET कैसे बनाएं?

**JWT_SECRET क्या है?**
यह एक लंबा random password है जो application के login system को secure करता है। इसे कभी किसी को न बताएं और न ही GitHub पर upload करें।

**बनाने के 4 तरीके:**

**तरीका 1 — Node.js से (सबसे आसान)**
```bash
node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"
```

**तरीका 2 — Docker Container से**
```bash
...
**तरीका 4 — OpenSSL से (Linux/Mac)**
```bash
openssl rand -hex 64
```

इनमें से कोई एक command चलाएं — जो output आए उसे copy करके `.env` file में `JWT_SECRET=` के बाद paste करें।
...
> ⚠️ हर नई deployment के लिए नया JWT_SECRET generate करें। ऊपर वाला example कभी use न करें।

---

### Server पर Deploy करने से पहले

```
☐  .env file में यह सब बदलें:
     ✏️  JWT_SECRET        → ऊपर बताए तरीके से नया generate करें
     ✏️  ADMIN_RECOVERY_SECRET → कोई याद रखने वाला secret phrase
     ✏️  FRONTEND_URL      → http://SERVER_IP (जैसे http://152.67.10.25)
     ✏️  SCHOOL_NAME       → स्कूल का असली नाम
     ✏️  SCHOOL_EMAIL      → स्कूल की असली email

☐  Server पर Port 80 खोलें (Firewall / Security Group में)

☐  docker-compose up -d --build चलाएं

☐  पहली बार: docker exec school-backend node seed.js
   (एक बार ही चलाएं — दोबारा चलाने पर data safe रहेगा)
```

### Client को Application देने से पहले

```
☐  Admin password बदला गया है (admin123 नहीं रहना चाहिए)

☐  WhatsApp connect हो गया है:
     Admin → WhatsApp Setup → Connected ✓ दिख रहा है

☐  Timetable सभी classes का बना दिया है

☐  Exam Schedule add कर दिया है

☐  छात्रों के records में Parent Phone number डाला है

☐  End-to-end test किया है:
     → एक छात्र Absent mark किया → Parent को WhatsApp आया ✅
     → एक Fee payment की → Parent को WhatsApp आया ✅
     → Fee Receipt PDF download की ✅
     → Student ID Card PDF download किया ✅
```

### Client को यह बताएं

```
1. Login URL:  http://SERVER_IP/login.html
2. Admin Login: admin@school.com (password जो आपने set किया)
3. Password भूलने पर: Admin Recovery Secret use करें
4. WhatsApp disconnect हो जाए तो: Admin → WhatsApp Setup → Reconnect
5. Backup के लिए: Server का snapshot लेते रहें (Oracle Cloud)
```
